...
export const requireAdmin = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (token !== 'admin-token-123') return res.status(401).json({ message: 'Invalid token' });

  // Fetch admin user
  const admin = await db.query.users.findFirst({ where: (users, { eq }) => eq(users.username, 'admin') });
